function Train_SVM(PH,L,cij,RusRatio,model,TrainingEvents,run)

rng(1);
DT=.1;    % Start-of-event threshold
RT=.8;    % End-of-event threshold

filename=sprintf('%s_%d_%s.mat',model,TrainingEvents,run);
    ...
    for jj=1:length(L)
        ...
        for ll=1:length(RusRatio)
            ...
            for kk=1:length(cij)
                ...
                [TP,FP,TN,FN]=getConfMatrix(predictions,Labels);
        ...
        if Lscore~=0
            predictions=predict(mdlOpt,vData);
